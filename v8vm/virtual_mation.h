...
#define PER_ISOLATE_PRIVATE_SYMBOL_PROPERTIES(V)                    \
    ...
    V(decorated_private_symbol, "v8vm:decorated")                   /* 异常是否被装饰，ContextifyScript::DecorateErrorStack() */
...
#define ENVIRONMENT_STRONG_PERSISTENT_PROPERTIES(V)                 \
    V(as_external, v8::External)                                    /* V8VirtualMation*的v8::External封装 */\
...
#define V(TypeName, PropertyName) inline v8::Local<TypeName> PropertyName() const;
...
#define VY(PropertyName, StringValue) V(v8::Symbol, PropertyName)
#define VS(PropertyName, StringValue) V(v8::String, PropertyName)
#define V(TypeName, PropertyName) v8::Eternal<TypeName> m_ ## PropertyName;
PER_ISOLATE_PRIVATE_SYMBOL_PROPERTIES(VP)
PER_ISOLATE_SYMBOL_PROPERTIES(VY)
PER_ISOLATE_STRING_PROPERTIES(VS)
#undef V
...
/****************************************************************************************************
* 为V8VirtualMation的Isolate定义一些属性
****************************************************************************************************/
#define VP(PropertyName, StringValue) V(v8::Private, PropertyName)
#define VY(PropertyName, StringValue) V(v8::Symbol, PropertyName)
#define VS(PropertyName, StringValue) V(v8::String, PropertyName)
